...
BENCHMARK_CASES = {
    ...
    "auxiliaires": {"description":"Les 6 auxiliaires","weight":2.0,"cases":[
        {"prompt":"Traduis en Dioula : Tu travailles.","expected":"I bɛ baara kɛ.","keywords":["bɛ","baara"],"aux":"bɛ présent affirmatif"},
        {"prompt":"Traduis en Dioula : Tu ne travailles pas.","expected":"I tɛ baara kɛ.","keywords":["tɛ","baara"],"aux":"tɛ présent négatif"},
        {"prompt":"Traduis en Dioula : Tu as travaillé.","expected":"I ye baara kɛ.","keywords":["ye","baara"],"aux":"ye passé affirmatif"},
        {"prompt":"Traduis en Dioula : Tu n'as pas travaillé.","expected":"I ma baara kɛ.","keywords":["ma","baara"],"aux":"ma passé négatif"},
        {"prompt":"Traduis en Dioula : Tu vas travailler.","expected":"I bɛna baara kɛ.","keywords":["bɛna","baara"],"aux":"bɛna futur affirmatif"},
        {"prompt":"Traduis en Dioula : Tu ne vas pas travailler.","expected":"I tɛna baara kɛ.","keywords":["tɛna","baara"],"aux":"tɛna futur négatif"},
    ]},
...
def bar(s, w=28):
    f=int(s*w); return f"[{'█'*f}{'░'*(w-f)}] {s*100:.0f}%"

def c(t,code): return f"\033[{code}m{t}\033[0m"

def main():
    p=argparse.ArgumentParser()
    p.add_argument("--model",default="./llama-3.2-3b-mlx")
    p.add_argument("--adapter",default="./adapters")
    ...
    print("\n"+"═"*65)
    print("  🎯 BENCHMARK DIOULA")
    print("═"*65)
    print(f"  Modèle : {args.model}\n  Adapter: {adapter or 'fusionné'}\n  Date   : {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    print("═"*65)
    ...
    results={}; tw=0.0; tt=0.0

    for cid,cat in BENCHMARK_CASES.items():
        desc=cat["description"]; w=cat["weight"]; cases=cat["cases"]
        print(f"\n{'─'*65}\n  📂 {desc.upper()}\n{'─'*65}")
        scores=[]; details=[]

        for i,case in enumerate(cases,1):
            prompt=case["prompt"]; expected=case["expected"]; kws=case["keywords"]
            aux=case.get("aux","")
            print(f"\n  [{i}/{len(cases)}] {prompt}")
            fp=build_prompt(tok,prompt)
            t1=time.time()
            try:
                resp=generate(model,tok,prompt=fp,max_tokens=args.max_tokens,verbose=False)
                for tk in ["<|eot_id|>","<|end_of_text|>","</s>"]:
                    resp=resp.replace(tk,"")
                resp=resp.strip()
            except Exception as e:
                resp=f"[ERREUR:{e}]"
            el=time.time()-t1
            ...
            if aux: print(f"  ✦ Auxiliaire : {aux}")
            details.append({"prompt":prompt,"expected":expected,"response":resp,"score":round(sc,3),"found":found})

        avg=sum(scores)/len(scores) if scores else 0.0
        tw+=avg*w; tt+=w
        bc="92" if avg>=0.8 else "93" if avg>=0.5 else "91"
        print(f"\n  Catégorie : {c(bar(avg),bc)}  (poids ×{w})")
        results[cid]={"description":desc,"weight":w,"score":round(avg,3),"details":details}

    gs=tw/tt if tt else 0.0
    print("\n"+"═"*65+"\n  📊 RÉSULTATS GLOBAUX\n"+"═"*65)
    for cid,r in results.items():
        s=r["score"]; icon="✅" if s>=0.8 else "⚠️ " if s>=0.5 else "❌"
    ...
    out.write_text(json.dumps({"date":datetime.now().isoformat(),"model":args.model,"global_score":round(gs,3),"categories":results},ensure_ascii=False,indent=2),encoding="utf-8")
    print(f"\n  💾 Rapport : {out}\n"+"═"*65+"\n")
